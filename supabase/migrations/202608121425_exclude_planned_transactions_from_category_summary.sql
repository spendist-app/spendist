...
as $$
  with filtered_transactions as (
    select
      t.id,
      t.category_id,
      coalesce(t.amount_in_default, t.amount) as amount
    from public.transactions t
    where t.owner_id = auth.uid()
      and t.direction = 'expense'
      and t.transaction_state = 'completed'
...
comment on function public.category_expense_summary(timestamptz, timestamptz)
  is 'Aggregates completed expense totals in wallet currency and counts per category and category group for the authenticated user within the provided date range.';
