...
`mcp.spendist.app` is a Cloudflare Custom Domain attached directly to the separate `spendist-mcp` Worker, not a DNS alias to the web application. `npm run mcp:worker:deploy` provisions the Custom Domain, DNS record, and certificate through Wrangler. The production workflow supplies the Supabase URL, publishable key, and derived OAuth issuer through an ephemeral secrets file during the same deploy, then verifies the health and OAuth metadata endpoints.
