...
- **Start Supabase**: `npm run supabase:start`
  - Wait for the CLI to report `Started supabase local development stack`. The stack runs in Docker containers on this app's local ports (API: 55321, DB: 55322, Studio: 55323).
...
1. Launch the Angular dev server: `npm run start`.
2. The start script checks whether this app's local Supabase stack is already running, verifies the configured ports are free if it is not, and starts it with `npx supabase start`.
